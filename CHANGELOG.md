...
- 所有工位、所有頻道都可在設定頁切換 `TC` 或 `5V Volt` 訊號源；既有工位 5 第 20 個通道 `0610` 預設為 5V。
- 新增 `ch_source` per-station/per-channel 設定，`gx20_reader.py` 依設定辨識電壓頻道；電壓頻道僅接受 `V`、`DC V`、`V DC` 單位。
- GX20 數值解析支援實機較長的電壓回應行，以及完整科學記號，例如 `+00003650E-03` → `3.65V`。
- 無效值（例如 `+99999999E-01`、`-9999999.9E+00`）會轉為 `None`。
- **v12.0 r1** 設定頁接點卡片「顯示」checkbox 旁新增 Volt 開關（toggle 樣式），切到 V 時主畫面該頻道顯示 3 位小數（讀值/平均/最大/最小），TC 維持原 1 位；`storage.js` save payload 與 dirty snapshot 都已涵蓋 `ch_source`，按保存後設定寫進 server 端 SQLite。
